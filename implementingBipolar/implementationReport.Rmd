...
```
Based on that I implemented it and executed it in R. You can see the repository with all my code [here](). 

## Preliminary evaluation of filtering algorithm using default parameters

As expected, the high speed between measurement is associated with high accuracy scores (that is, inaccurate measurements). Approximately x percent of measurements in all of 2017 were associated with such jumps.

What Nick does is exclude those points if they are often associated with big jumps (i.e. more than ten times).

For my own data in 2017 there was only one location which was excluded (a total of x measurements), which means it is not a very effective filtering algorithm using the original parameters. Moreover, the points which are filtered depend on the length of the data which is taken into account.
